...
- **Lin's CCC and Spearman** (from `app/metric.py`) are computed pairwise for every unique pair among the selected models (10 pairs across all 5 models). Four matrices are produced: Action×CCC, Action×Spearman, Consequence×CCC, Consequence×Spearman. CCC (not Pearson) was chosen deliberately, for the same reason it's the primary Human-LLM Alignment metric in §2.1: two models could rank scenarios identically while scoring them on systematically different scales, which Pearson would miss and CCC would not — and using the same metric on both sides makes "models agree with each other more than with the human" a fair, same-metric comparison rather than one comparing CCC against Pearson.
